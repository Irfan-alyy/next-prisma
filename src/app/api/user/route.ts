...
export async function PUT(request: Request) {
  ...
  if (removeImage) {
    const imagePath = join(process.cwd(), 'public', 'profile_pictures', session?.user?.image?.split('/').pop() || '');
    try {
  ...
  if (imageFile && imageFile.size > 0) {
    ...
    if (currentUser?.image) {
      const imagePath = join(process.cwd(), 'public', 'profile_pictures', currentUser.image.split('/').pop() || '');
      try {
        await unlink(imagePath);
      } catch (error) {
        console.warn('Failed to delete image file:', error);
